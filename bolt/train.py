...
from models.reasoning import id2rel
from models.reasoning import ImageTool
...
DATA_INPUT = '/yq/ddd/intel_amm_2/data-end2end-triple/'
...
def train(model):
    ...
    train_set = TripleDataset(DATA_INPUT + 'triples_con_train.tsv')
    test_set = TripleDataset(DATA_INPUT + 'triples_con_test.tsv')  # use triple
    ...
    for epoch in range(epoch_num):
        ...
        best_score = 0.3
        for i in index_list:
            (img, op_list, answer, type, name_t) = train_set[i]
            # print(op_list)
            # print(op_list)
            # optimizer.zero_grad()
            # start_time = time.time()
            # print("start time begin")
            y_pred = model(op_list, img, name_t, mode='train')
        ...
        for i in range(test_set.len):
            (img, ann, op_list, answer, type, name_t, id_a, id_b) = test_set[i]
            y_pred = model(op_list, img, ann, name_t, id_a, id_b, mode='train')
            loss = loss_function(y_pred, answer)
            ...
            y_pred = model(op_list, img, ann, name_t, id_a, id_b, mode='test')
            # acc compute
...
def infer_checkpoint(model):
    ...
    for img_id in img_list:
        ...
        y_pred = model(op_list, img, ann, mode='infer')
...
class TripleDataset(Dataset):
    def __init__(self, file):
        ...
        with open(file) as f:
            ...
            for i, line in enumerate(lines):
                if i == 0:
                    continue
                (img_id, attribute_in, attribute_out, attribute_cor) = line.split(' ')  # dataset triple 格式
                triples_pos.append((int(img_id), int(attribute_in), int(attribute_out), int(attribute_cor)))
        ...
        for triple in triples_pos:
            ...
            question = {
                ...
                "op_list": [
                    ...
                    {"op": "obj_attibute", "param": attribute_list}],  # 通过filter判定该物体是否是triple中subject物体
    ...
    def __getitem__(self, index):
        ...
        return (img, op_list, answer, type, name_t)
